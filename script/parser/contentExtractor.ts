...
// @ts-ignore
export const extractItem = async (elem,pathPrefix:string,imgPath:string,images:MDImage[]): string => {
    let ans = ""
    if (elem?.data) {
    ...
    switch (elem.name) {
        ...
        case "h2":
            ans += "#"
        case "h1":
            ans += "# " + await extractItem(elem.children,pathPrefix,imgPath,images) + "\n"
            return ans
        case "table":
            let text = "\n\n"
            // @ts-ignore
            for (let item of elem.children) {
                text += await extractItem(item,pathPrefix,imgPath,images)
            }
            return text
        case "tbody":
            let body = ""
            let w = elem.children[0].children.length
            body += `${await extractItem(elem.children[0],pathPrefix,imgPath,images)}\n`
            body += "|" + Array(w).fill("---").join("|") + "|\n";
            for (let item of elem.children.slice(1)) {
                body += await extractItem(item,pathPrefix,imgPath,images) + "\n";
            }
            return body
        case "tr":
            ans = "|"
